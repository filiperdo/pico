...
class Data
{
	...
	/**
	 * Obtem as imagens de todos os formatos 
	 * de uma determinada pasta
	 * @param unknown $type
	 * @param unknown $id
	 * @return unknown[]
	 */
	static public function getImgPost( $path, $thumb = NULL )
	{
		$array_img = array();
		
		if( $thumb )
		{
			foreach( glob('public/img/'. $path .'/thumb/*.*' ) as $key => $imagem )
			{
				$array_img[] = $imagem;
			}
		}
		else {
			foreach( glob('public/img/'. $path .'/*.*' ) as $key => $imagem )
			{
				$array_img[] = $imagem;
			}
		}
